...
The Microsoft connection uses Graph rather than IMAP; this is intentionally invisible in the normal Inkwell workflow. Graph sync imports up to 200 inbox messages and discovers the server folder hierarchy. Selecting a server folder imports up to its newest 200 messages. See [Mail workspace](MAIL-WORKSPACE.md). Import uses immutable IDs, requests plain-text bodies, strips unexpected HTML and deduplicates retained local copies. Attachments are not downloaded. Read flags, stars and folders are still local-only after import. Next-page URLs must remain on the Graph origin; tokens are not forwarded to arbitrary URLs.
