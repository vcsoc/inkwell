...
Schema 15 adds per-message JSON attachment metadata/checkpoints with cascading local deletion, account/remote-key identity binding, and exact Graph conversation discovery. File bytes are not permanently cached. Native ICS file opening uses a bounded queue of explicitly supplied local paths, a trusted-main-document-only consume capability, and the existing import review; it does not expose arbitrary filesystem reads to the renderer.
...
3. Attachment sending, malware scanning, cached file storage, attached Outlook item/cloud downloads, and CID image rendering.
