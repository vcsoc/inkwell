...
Each color uses a compact **hex field with a swatch/dropdown button on its right**. The dropdown provides a saturation/brightness area, hue control and palette swatches, with mouse, touch and arrow-key support. It floats above the form instead of expanding underneath the field; Escape or an outside click dismisses it. There are no separate RGB channel sliders. There is no native color-dialog or desktop eyedropper invocation: no Wayland screen-capture chooser or screen-sharing permission is needed. Sampling arbitrary desktop pixels is not provided.
...
**Saved themes** appears beneath the live preview. Saving a theme adds it to the library (saving again with the same name updates it). Each thumbnail shows its background, surface and accent; Apply activates that theme immediately and persists it across restarts, while Delete removes it from the library without changing the current appearance. Imported themes join the library when saved. The existing active theme is retained as the first saved theme when upgrading. Saved themes and presentation preferences are loaded on startup by connected devices, not pushed live to already-open windows.
