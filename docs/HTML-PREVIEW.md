...
The backend uses **nh3** (an HTML5-aware sanitizer) with explicit allowed HTML tags, attributes and CSS properties. Mail is never inserted directly into the app's DOM. The preview is a separate authenticated HTML response in an iframe with an empty `sandbox` attribute (no scripts, same-origin privilege, forms, navigation privileges or popups). Its response also has a sandbox CSP, `script-src 'none'`, `default-src 'none'`, restricted inline styles, `base-uri 'none'`, `form-action 'none'`, frame-ancestor restrictions and no-referrer policy. The main app's CSP remains unchanged.
...
Scripts, forms, SVG/MathML documents, embedded frames/objects, CSS image URLs, external stylesheets, fonts, audio/video and email navigation links remain disabled even after image permission. Inline presentational styles are limited. Layout will not exactly match every newsletter. This is not an antivirus, a guarantee against all browser vulnerabilities, or a guarantee that an email's claims are trustworthy. Up to 50 eligible origins can be offered in one view; other resources remain blocked.
