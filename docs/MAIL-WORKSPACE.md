...
An initial quick pull imports up to **200 messages**. A shared background worker then visits every discovered Outlook folder and subfolder, following message pagination without requiring folder clicks. It processes one folder at a time, exposing current folder, progress and failures while keeping the UI usable. Initial backfill follows at most 5,000 pages (up to 500,000 messages per folder); hitting this safety limit reports an incomplete sync rather than silently claiming completion. Each imported page is retained if a later request fails. Completed folders use newest-200 checks on subsequent automatic passes, approximately every two minutes while an app is open. Failed folders retry their backfill. Restarting the backend rescans from the beginning with duplicate-safe imports; there is no persisted pagination cursor. F9 requests a full rescan when no job is active. Large mailboxes can take time and disk space; no attachments are downloaded. Server drafts and sent folders are viewable as imported mail, not editable server drafts. Generic IMAP folder discovery is not implemented yet; the current tree implementation is for Microsoft Graph. See [background sync](BACKGROUND-SYNC.md) for job APIs, backfill limits, retries and shutdown behavior.
...
Press **F9** (or click **Sync email**) to request a fresh server pull immediately. An open server folder syncs that folder; other views use the connected accounts' normal inbox sync, including existing discovery/trusted-Junk handling. It then requests the all-folder background backfill/rescan described above. Existing safety limits and read-only provider protections still apply; it never bypasses an active sync lock. Held-key repeats and modified F9 shortcuts are ignored; overlapping manual requests are suppressed and failures leave the shortcut available for retry.
