...
- Calendar surfaces, foregrounds, event colors and controls follow the selected theme. The month grid uses the available application height instead of sitting above a long agenda. The agenda starts collapsed in a narrow right-hand panel; select **Agenda** to expand its independently scrolling list and export .ics, then collapse it to return the grid to full width. On phones the expanded panel overlays the grid. The range instructions remain available to screen readers and on the date-range button, without occupying a visible banner. The existing Popup/Internal form preference and wide-screen calendar side editor still apply; there are no additional week/day views.
